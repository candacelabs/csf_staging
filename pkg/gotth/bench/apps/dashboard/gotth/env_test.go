...
func flagsUnderTest() *flag.FlagSet {
	fs := flag.NewFlagSet("dashboard-gotth", flag.ContinueOnError)
	...
	fs.String("htmx", DefaultHTMXPath, "path to htmx.min.js")
	fs.String("fixtures", DefaultFixtureDir, "path to bench/fixtures")
	fs.Duration("tick", TickMs*time.Millisecond, "fixture replay interval")
	return fs
}
...
var _ = Describe("§5.2's container environment, translated into this app's flags", func() {
	...
	// Region E is plain HTMX on this stack (§2.4, AS-3) and LoadHTMX verifies
	// the bundle's digest before it will serve it, so the path has to reach the
	// binary or the process refuses to start at all.
	It("takes the vendored HTMX bundle's path from BENCH_HTMX_PATH", func() {
		Expect(fs.Parse(nil)).To(Succeed())
		Expect(applyEnvFallbacks(fs, envOf(map[string]string{
			"BENCH_HTMX_PATH": "/bench/vendor/htmx-2.0.10.min.js",
		}))).To(Succeed())
		Expect(value("htmx")).To(Equal("/bench/vendor/htmx-2.0.10.min.js"))
	})

	It("takes the fixture directory from BENCH_FIXTURE_DIR, which is compose's read-only mount", func() {
		Expect(fs.Parse(nil)).To(Succeed())
		Expect(applyEnvFallbacks(fs, envOf(map[string]string{
			"BENCH_FIXTURE_DIR": "/bench/fixtures",
		}))).To(Succeed())
		Expect(value("fixtures")).To(Equal("/bench/fixtures"))
	})

	Describe("§2.5's replay interval", func() {
		It("reads BENCH_TICK_MS as the bare count of milliseconds it is named for", func() {
			Expect(fs.Parse(nil)).To(Succeed())
			Expect(applyEnvFallbacks(fs, envOf(map[string]string{"BENCH_TICK_MS": "10"}))).To(Succeed())
			Expect(value("tick")).To(Equal("10ms"))
		})

		// Refusing beats guessing: a replay running at the wrong rate produces
		// a run whose manifest records a schedule the server did not keep.
		It("refuses a value that is not a bare count rather than guessing at it", func() {
			Expect(fs.Parse(nil)).To(Succeed())
			err := applyEnvFallbacks(fs, envOf(map[string]string{"BENCH_TICK_MS": "100ms"}))
