...
func (service *Service) registerOperations() {
	...
	registerOperation(service, "LearnAboutCSF", "POST", "/api/onboarding/learn", "Explain the pinned CSF contract and index explicitly selected onboarding sources for retrieval.", json.RawMessage("{\"properties\":{\"consumerPaths\":{\"items\":{\"type\":\"string\"},\"type\":\"array\"},\"copilotSessionIds\":{\"description\":\"Explicitly selected native Copilot CLI sessions. The host must opt in by\\nconfiguring the read-only SDK bridge; an empty list performs no history read.\",\"items\":{\"type\":\"string\"},\"type\":\"array\"},\"retrievalQuery\":{\"type\":\"string\"}},\"type\":\"object\"}"), func() *contract0.LearnAboutCSFRequest { return &contract0.LearnAboutCSFRequest{} }, service.LearnAboutCSF)
...
func (client *Client) LearnAboutCSF(ctx context.Context, request *contract0.LearnAboutCSFRequest) (*contract0.LearnAboutCSFResponse, error) {
	response := &contract0.LearnAboutCSFResponse{}
	if err := client.call(ctx, "POST", "/api/onboarding/learn", request, response); err != nil {
		return nil, err
	}
	return response, nil
}
...
func (client *Client) CallOperation(ctx context.Context, operation string, input []byte) ([]byte, error) {
	...
	case "search":
		...
		return (protojson.MarshalOptions{UseProtoNames: true}).Marshal(response)
	case "learnaboutcsf":
		request := &contract0.LearnAboutCSFRequest{}
		if err := protojson.Unmarshal(input, request); err != nil {
			return nil, err
		}
		response, err := client.LearnAboutCSF(ctx, request)
		if err != nil {
			return nil, err
		}
		return (protojson.MarshalOptions{UseProtoNames: true}).Marshal(response)
