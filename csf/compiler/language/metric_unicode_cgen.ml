(* Pinned compatibility data derived from Python 3.12's Unicode 15.0.0
   str.isalnum() classification, plus underscore. This checked-in table is the
   runtime authority; change it only as part of an explicit Unicode migration. *)
let word_ranges = [|
...
let is_word point =
  let rec search first last =
    if first > last then false
      ...
      let lower, upper = word_ranges.(middle) in
      if point < lower then search first (middle - 1)
      else if point > upper then search (middle + 1) last
      else true in
  search 0 (Array.length word_ranges - 1)
