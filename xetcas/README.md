...
```bash
cd xetcas          # this directory, from the repository root
bash demo/demo.sh
```
...
These crates are also part of the repository-wide Bazel module rooted one
level up, at `candace/`. Nothing about the cargo workspace changed to make
that happen: `MODULE.bazel` hands crate_universe the same `Cargo.toml` and
`Cargo.lock` cargo reads, pins rustc to 1.91.1 (what `rust:1.91-bookworm`
ships, so both paths compile with one compiler), and writes to neither file.
cargo stays the path the demo, the container images, and every `just` target
above take.

```bash
cd ..                                 # the Bazel module root
./tools/bazel.sh build //xetcas/...   # xetcasd, the contracts, xtask
./tools/bazel.sh test //xetcas/...    # every test but git_hosting
```

`tools/bazel.sh` runs a pinned Bazel container, so that needs Docker and
neither Bazel nor Rust on the host. Two things stay outside it. Codegen: the
committed output under `crates/xetcas-contracts/src/generated/` is what Bazel
compiles, and `cargo xtask gen-proto` is still what produces it. And
`//xetcas/crates/xetcasd:git_hosting_test`, which shells out to `git` — the
Bazel image has no copy, so the target is tagged `manual` and `cargo test` is
its authority.
...
its budget. [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) builds
and tests `//xetcas/...` on every push to this repository; formatting, clippy,
codegen drift, and the workbench image build run in the monorepo this tree is
exported from, before a snapshot is ever published.
