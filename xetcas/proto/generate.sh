...
#
# The Go bindings are part of the repository-wide Go module, and the Liquid
# Proto plugin is built from this same checkout, so the container gets the
# repository root rather than the component directory.
set -euo pipefail
...
docker run --rm \
  ...
  "${toolchain_image}" \
  ./candace/xetcas/proto/generate-in-container.sh "${mode}"
