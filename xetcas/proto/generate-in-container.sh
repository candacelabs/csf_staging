...
repo_root=/workspace
component_root="${repo_root}/candace/xetcas"
# liquidproto/v1/refinement.proto lives here, and this -I root keeps the
# descriptor file name ("liquidproto/v1/refinement.proto") stable.
liquidproto_root="${repo_root}/candace/pkg"
mode="${1:-write}"
case "${mode}" in
  write)
...
# The Rust codegen path (protox) cannot read the canonical file through the
# repository-wide -I roots, so it compiles against
# proto/vendor/liquidproto/v1/refinement.proto instead. Fail loudly if that
# vendored copy has drifted from the canonical one.
canonical_refinement="${liquidproto_root}/liquidproto/v1/refinement.proto"
vendored_refinement="${component_root}/proto/vendor/liquidproto/v1/refinement.proto"
...
diff -u \
  --label "candace/pkg/liquidproto/v1/refinement.proto" \
  --label "candace/xetcas/proto/vendor/liquidproto/v1/refinement.proto (below marker)" \
  "${canonical_refinement}" "${vendored_body}"

export GOCACHE=/tmp/xetcas-proto-gocache
export GOMODCACHE=/tmp/xetcas-proto-modcache
export GOPATH=/tmp/xetcas-proto-gopath

# The Liquid Proto plugin is first-party and lives in this checkout, so build it
# here rather than installing a published copy: that is what keeps the emitted
# runtime import path in step with where liquidproto actually lives.
plugin_dir="$(mktemp -d /tmp/xetcas-liquid-plugin.XXXXXX)"
...
module_path=github.com/candacelabs/csf/xetcas

protoc \
  -I "${component_root}/proto" \
  -I "${liquidproto_root}" \
  -I /usr/local/include \
  "--go_out=module=${module_path}:${output_root}" \
  "--plugin=protoc-gen-liquidproto=${plugin_dir}/protoc-gen-liquidproto" \
  "--liquidproto_out=module=${module_path}:${output_root}" \
  "${proto_files[@]}"
...
# never examined, and the exported Go bindings keep publishing stale messages
# and validators while drift reports clean.
#
